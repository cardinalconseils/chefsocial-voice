...
module.exports = (app) => {
    ...
    router.post('/webhook/image-received', 
        ...
        asyncHandler(async (req, res) => {
            ...
            logger.info('Twilio webhook /image-received called', {
                method: req.method,
            ...
            const imageUrl = parsedBody.get('MediaUrl0'); // First image attachment
            ...
            logger.info('MMS webhook parsed data', {
            ...
            if (!fromNumber || !messageSid) {
                logger.error('Invalid Twilio webhook - missing required fields', {
                    hasFrom: !!fromNumber,
            ...
            try {
                ...
                if (imageUrl && numMedia > 0) {
                    logger.info('Processing MMS with image - starting briefing session', {
                        fromNumber: fromNumber.slice(-4),
                    ...
                    logger.info('Briefing session created - sending scheduling options', {
                        fromNumber: fromNumber.slice(-4),
                    ...
                    logger.info('MMS workflow completed successfully', {
                        fromNumber: fromNumber.slice(-4),
                        sessionId: result.sessionId,
                        messageSid,
                    ...
                    logger.warn('MMS received without image - sending help message', {
                        fromNumber: fromNumber.slice(-4),
                        numMedia,
                    ...
                    const helpResponse = await smsService.sendResponse(fromNumber, 
                    ...
                    logger.info('Help message sent for imageless MMS', {
                        fromNumber: fromNumber.slice(-4),
                ...
                try {
                    const errorResponse = await smsService.sendResponse(fromNumber, 
    ...
    router.post('/webhook/schedule-response', 
        ...
        asyncHandler(async (req, res) => {
            ...
            logger.info('Twilio webhook /schedule-response called', {
                method: req.method,
                contentType: req.headers['content-type'],
                userAgent: req.headers['user-agent'],
                twilioSignature: req.headers['x-twilio-signature'],
            ...
            // Enhanced logging with all Twilio webhook data
            logger.info('Schedule response webhook parsed data', {
                fromNumber: fromNumber ? fromNumber.slice(-4) : 'missing',
                toNumber: toNumber ? toNumber.slice(-4) : 'missing',
                messageSid: messageSid || 'missing',
                accountSid: accountSid ? accountSid.substring(0, 10) + '...' : 'missing',
            ...
            if (!fromNumber || !messageSid || !messageBody) {
                logger.error('Invalid schedule response webhook - missing required fields', {
                    hasFrom: !!fromNumber,
                    hasMessageSid: !!messageSid,
            ...
            try {
                ...
                logger.info('Processing schedule response from user', {
                    fromNumber: fromNumber.slice(-4),
                    response: messageBody,
                    messageSid
                });
                ...
                if (result.success) {
                    logger.info('Schedule response processed successfully', {
                        fromNumber: fromNumber.slice(-4),
                        sessionId: result.sessionId,
                    ...
                    const session = await authSystem.db.getBriefingSession(result.sessionId);
                    
                    // Trigger N8N workflow for content generation
                    if (app.locals.services.n8nCoordinator && session) {
                        try {
                            logger.info('Triggering N8N workflow for content generation', {
                                sessionId: result.sessionId,
                                fromNumber: fromNumber.slice(-4),
                                scheduledTime: result.scheduledTime
                            });
                        ...
                        logger.warn('N8N coordinator or session not available', {
                    ...
                    logger.warn('Schedule response processing returned unsuccessful result', {
                        fromNumber: fromNumber.slice(-4),
                        messageBody,
                        result
                    });
    ...
    // POST /api/sms/test-webhook - Test webhook configuration
    router.post('/test-webhook', 
        ...
        asyncHandler(async (req, res) => {
            ...
            logger.info('Webhook test data parsed', testData);
    ...
    // GET /api/sms/webhook-logs - Get recent webhook activity logs
    router.get('/webhook-logs', 
        ...
        asyncHandler(async (req, res) => {
            ...
            logger.info('Webhook logs accessed', {
                userId: req.userId,
